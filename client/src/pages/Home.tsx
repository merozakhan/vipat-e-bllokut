                  ...
                  <div className="group relative overflow-hidden rounded-xl cursor-pointer">
                    ...
                    <div className="absolute bottom-0 left-0 right-0 p-4 md:p-8">
                      <div className="flex items-center gap-2 mb-2 md:mb-3">
                        <span className="px-2 md:px-3 py-0.5 md:py-1 bg-gold text-navy-dark text-[10px] md:text-xs font-bold uppercase tracking-wider rounded font-sans">
                          Featured
                        </span>
                        <span className="px-2 md:px-3 py-0.5 md:py-1 bg-white/10 backdrop-blur-sm text-white/90 text-[10px] md:text-xs font-medium rounded font-sans">
                          {formatDate(featuredArticle.publishedAt)}
                        </span>
                      </div>
                      <h2 className="text-lg md:text-3xl lg:text-4xl font-bold text-white leading-tight mb-2 md:mb-3 group-hover:text-gold-light transition-colors line-clamp-3">
                        {featuredArticle.title}
                      </h2>
                      {featuredArticle.excerpt && (
                        <p className="hidden md:block text-white/70 text-sm md:text-base leading-relaxed line-clamp-2 font-sans max-w-2xl">
                          {featuredArticle.excerpt}
                        </p>
                      )}
                ...
                {sideArticles.length > 0 ? (
                  sideArticles.map((article, index) => (
                    <Link key={article.id} href={`/article/${article.slug}`}>
                      <div className="group flex gap-3 md:gap-4 p-3 md:p-4 bg-card rounded-lg border border-border/50 hover:border-gold/30 transition-all cursor-pointer premium-card">
                        <div className="flex-shrink-0 w-8 h-8 md:w-10 md:h-10 rounded-full bg-secondary flex items-center justify-center">
                          <span className="text-gold font-bold text-base md:text-lg font-sans">{index + 1}</span>
                        </div>
                        <div className="flex-1 min-w-0">
                          <h4 className="text-xs md:text-sm font-semibold text-foreground leading-snug line-clamp-2 group-hover:text-gold transition-colors">
                            {article.title}
                          </h4>
                          <div className="flex items-center gap-3 mt-1.5 md:mt-2 text-[10px] md:text-xs text-muted-foreground font-sans">
        ...
        <div className="container">
          ...
          <div className="flex items-center justify-between mb-6 md:mb-8">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-xl md:text-3xl font-bold text-foreground">Latest News</h2>
                <p className="text-[10px] md:text-xs text-muted-foreground font-sans mt-1 uppercase tracking-wider">Stay informed with our latest coverage</p>
              </div>
            </div>
          </div>
                ...
                <Link key={article.id} href={`/article/${article.slug}`}>
                  <div className="group bg-card rounded-lg md:rounded-xl overflow-hidden border border-border/50 hover:border-gold/30 transition-all cursor-pointer premium-card h-full flex flex-col">
                    ...
                    <div className="p-3 md:p-5 flex-1 flex flex-col">
                      <h3 className="text-sm md:text-lg font-bold text-card-foreground mb-1 md:mb-2 leading-snug line-clamp-2 group-hover:text-gold transition-colors">
                        {article.title}
                      </h3>
                      ...
                      <div className="flex items-center justify-between text-[10px] md:text-xs text-muted-foreground mt-auto pt-2 md:pt-4 border-t border-border/50 font-sans">
                        <span className="flex items-center gap-1">
                          ...
                          <span className="sm:hidden">{new Date(article.publishedAt || "").toLocaleDateString("en-GB", { day: "numeric", month: "short" })}</span>
                        </span>
                        <span className="flex items-center gap-1">
        ...
        <div className="container">
          <div className="max-w-2xl mx-auto text-center">
            ...
            <a
              ...
              className="inline-flex items-center gap-2 px-6 md:px-8 py-2.5 md:py-3 bg-gold text-navy-dark font-bold text-xs md:text-sm uppercase tracking-wider rounded-lg hover:bg-gold-light transition-colors font-sans"
            >
