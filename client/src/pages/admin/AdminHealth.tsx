    ...
    <AdminLayout>
      ...
      {health && (
        <div className="space-y-4">
          {/* Overall Status */}
          <div className={`p-4 md:p-5 rounded-xl border ${isHealthy ? "bg-green-500/5 border-green-500/20" : isDegraded ? "bg-yellow-500/5 border-yellow-500/20" : "bg-red-500/5 border-red-500/20"}`}>
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-base md:text-lg font-bold text-foreground capitalize">{health.status}</h2>
                  <p className="text-[10px] md:text-xs text-muted-foreground">Uptime: {formatUptime(health.uptime)}</p>
                </div>
              </div>
              <span className="text-[10px] md:text-xs text-muted-foreground hidden sm:block">{new Date(health.timestamp).toLocaleString()}</span>
            </div>
          </div>

          {/* Database */}
          <div className="p-4 md:p-5 bg-card/50 rounded-xl border border-border/30">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 md:gap-4">
              <div className="p-3 bg-background/50 rounded-lg">
                <p className="text-xl md:text-2xl font-black text-foreground">{health.database.publishedArticles}</p>
                <p className="text-[9px] md:text-[10px] text-muted-foreground uppercase tracking-wider font-sans">Published</p>
              </div>
              <div className="p-3 bg-background/50 rounded-lg">
                <p className="text-xl md:text-2xl font-black text-foreground">{health.database.categories}</p>
                <p className="text-[9px] md:text-[10px] text-muted-foreground uppercase tracking-wider font-sans">Categories</p>
              </div>
              <div className="p-3 bg-background/50 rounded-lg">
                <p className={`text-xl md:text-2xl font-black ${health.database.sizeMb > health.database.maxSizeMb * 0.8 ? "text-red-400" : "text-foreground"}`}>{health.database.sizeMb}</p>
                <p className="text-[9px] md:text-[10px] text-muted-foreground uppercase tracking-wider font-sans">MB / {health.database.maxSizeMb}</p>
              </div>
              <div className="p-3 bg-background/50 rounded-lg">
                ...
                <p className="text-[9px] md:text-[10px] text-muted-foreground uppercase tracking-wider font-sans">DB Usage</p>
              </div>
            </div>
          </div>
          ...
          <div className="p-4 md:p-5 bg-card/50 rounded-xl border border-border/30">
            ...
            <div className="grid grid-cols-2 gap-3 md:gap-4">
              <div className="p-3 bg-background/50 rounded-lg">
                <p className="text-xs md:text-sm font-bold text-foreground">{health.maintenance?.wipeSchedule || "Not set"}</p>
                <p className="text-[9px] md:text-[10px] text-muted-foreground uppercase tracking-wider font-sans">Wipe Schedule</p>
              </div>
              <div className="p-3 bg-background/50 rounded-lg">
                <p className="text-xs md:text-sm font-bold text-foreground">{health.maintenance?.lastWipe ? formatTimeAgo(health.maintenance.lastWipe) : "Never"}</p>
                <p className="text-[9px] md:text-[10px] text-muted-foreground uppercase tracking-wider font-sans">Last Wipe</p>
              </div>
            </div>
          </div>
